...
export default function AppDashboard() {
  ...
  const [actionLoading, setActionLoading] = useState<string | null>(null); // TRACKS COMPONENT INSTANCE LOCKS
  ...
  const executeVoucherApproval = async (phone: string, tierKey: string, routerInstance: string) => {
    if (actionLoading) return; // SPAM BLOCK
    setActionLoading(phone); // Lock the individual row row
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.tunnel_online) {
          alert(`Approval complete. Device logged in automatically.`);
        } else {
      ...
      setActionLoading(null); // Clear context lock safely
    }
  };
